...
export default function ARPage() {
  ...
  useEffect(() => {
    ...
    bg.renderOrder = -1; bgScene.add(bg)
    ...
    function shoot() {
      ...
      if (hits.length > 0) {
        ...
        setHit(true); setTimeout(() => setHit(false), 200)
    ...
    function loop() {
      ...
      if (video.readyState >= 2 && vCtx) { vCtx.drawImage(video, 0, 0, W, H); vTex.needsUpdate = true }
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'#000', overflow:'hidden' }}>
      ...
      <div style={{ position:'absolute', top:0, left:0, right:0, display:'flex', justifyContent:'space-between', padding:'1rem 1.5rem', fontFamily:'monospace', fontSize:'11px', color:'#cc0000', pointerEvents:'none' }}>
      ...
      <div style={{ position:'absolute', bottom:'2rem', right:'1.5rem', fontFamily:'monospace', fontSize:'10px', color:'#cc0000', pointerEvents:'none' }}>
        {ammo === 0 ? 'RELOADING...' : 'AMMO: ' + String(ammo).padStart(2,'0') + ' / 12'}
      </div>
      {hit && <div style={{ position:'absolute', top:'40%', left:'50%', transform:'translate(-50%,-50%)', fontFamily:'monospace', fontSize:'14px', color:'#fff', pointerEvents:'none' }}>TARGET ELIMINATED</div>}
    </div>
  )
}
